      ...
      <div className="container mx-auto px-4">
        ...
        <section className="mb-8">
          ...
          <div className="bg-gray-200 p-6 rounded-lg">
            <h2 className="text-2xl font-semibold mb-2">{featuredNews.title}</h2>
            <p className="mb-4">{featuredNews.summary}</p>
            <Link href={`/news/${featuredNews.slug}`} className="text-blue-600 hover:underline">Read more</Link>
          </div>
        </section>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {topStories.map((story) => (
              <div key={story.id} className="border p-4 rounded-lg">
                ...
                <Link href={`/news/${story.slug}`} className="text-blue-600 hover:underline">Read more</Link>
              </div>
            ))}
          </div>
        </section>
